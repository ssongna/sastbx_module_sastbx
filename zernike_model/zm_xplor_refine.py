...
def run(args):
  ...
  zm_xplor_refine( data, start_file, rmax, qmax=qmax, nmax=nmax, np_on_grid=np, prefix=prefix, splat_range=splat_range, pdb_nlm=pdb_nlm, n_trial=n_trial )


class zm_xplor_refine(object):
  def __init__(self, data, xplor_file, rmax, qmax=0.15, nmax=20, np_on_grid=30, prefix='prefix', splat_range=1, n_trial=5, fraction=0.9, pdb_nlm=None):
    self.raw_data=data
