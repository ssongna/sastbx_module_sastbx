...
def run(args):
  ...
  zm_xplor_refine( data, start_file, rmax, qmax=qmax, nmax=nmax, np_on_grid=np_on_grid, prefix=prefix, splat_range=splat_range, pdb_nlm=pdb_nlm, n_trial=n_trial, nbr_dist=nbr_dist )


class zm_xplor_refine(object):
  def __init__(self, data, xplor_file, rmax, qmax=0.15, nmax=20, np_on_grid=30, prefix='prefix', splat_range=1, n_trial=5, fraction=0.9, pdb_nlm=None, nbr_dist=2):
    self.raw_data=data
...
## Randomly pick one point ##
    ...
    if(random.random()<0.1): # 5% of reversel modification
